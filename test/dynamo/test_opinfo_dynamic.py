...
def check_model(
    ...
    *,
    ...
    # run = torch._dynamo.optimize("aot_eager", nopython=True, dynamic=True)(run)
    # run = torch._dynamo.optimize(backend, nopython=True, dynamic=True)(run)
    run = torch._dynamo.optimize(backend, dynamic=True)(run)
    torch.manual_seed(0)
